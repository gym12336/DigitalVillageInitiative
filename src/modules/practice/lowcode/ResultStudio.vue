<template>
  <section class="studio tool-interface">
    <!-- 顶部工具条 -->
    <header class="toolbar">
      ...
      <div class="tb-actions">
        <button class="btn ghost" :class="{ on: preview }" @click="preview = !preview">
          <AppIcon :name="preview ? 'document' : 'eye'" :size="14" />{{ preview ? '编辑' : '预览' }}
        </button>
        <button class="btn ghost" @click="onSave"><AppIcon name="archive" :size="14" />保存</button>
        <button class="btn ghost" @click="onExportJSON"><AppIcon name="download" :size="14" />JSON</button>
        <button class="btn primary" @click="onExportSite"><AppIcon name="upload" :size="14" />导出网站</button>
      </div>
    </header>
    ...
    <div class="cols">
      ...
      <aside class="col-left" :class="{ hidden: preview }">
        <div class="panel-scroll">
          ...
          <div class="comp-list">
            <button
              ...
              @click="addByClick(p)"
            >
              <AppIcon class="comp-icon" :name="presetIcon(p.key)" :size="16" />{{ p.name }}
            </button>
          </div>
        ...
        <!-- AI 助手：本期预留接缝，不实现 -->
        <div class="ai-seam">
          <div class="ai-head"><AppIcon name="lightbulb" :size="16" />AI 助手</div>
        ...
        >
          ...
          >
            ...
            <div v-if="!preview" class="cell-tools">
              ...
              <button class="ct-btn del" title="删除" @click.stop="remove(node.id)"><AppIcon name="close" :size="12" /></button>
            </div>
          </div>

          <div v-if="!work.blocks.length" class="canvas-empty">
            <p><AppIcon name="seedling" :size="18" />从左侧把组件拖进来，或点击组件即可加入画布</p>
          </div>
        </div>
      </main>
...
function presetIcon(key) {
  ...
  if (value.includes('image') || value.includes('gallery')) return 'image'
  if (value.includes('map') || value.includes('route')) return 'map-pin'
  if (value.includes('people') || value.includes('quote')) return 'user'
  if (value.includes('metric') || value.includes('chart') || value.includes('kpi')) return 'chart'
  if (value.includes('timeline')) return 'practice'
  return 'document'
}
...
.tb-actions .btn { display: inline-flex; align-items: center; gap: .35rem; }
...
.comp-icon { color: var(--jade); }

/* AI 接缝 */
.ai-seam { padding: 1rem; border-top: 1px solid var(--color-border); background: var(--color-bg); }
.ai-head { display: flex; align-items: center; gap: .4rem; font-size: .84rem; font-weight: 700; color: var(--color-primary-dark); margin-bottom: .4rem; }
...
.canvas-empty p { display: flex; align-items: center; gap: .5rem; }
