<template>
  <section class="detail-page museum-public-page">
    <MuseumPageHero
      archive-no="FIELD DEMAND · DETAIL"
      kicker="乡村之声 / 需求档案"
      :title="demand?.title || (error ? '需求档案暂不可用' : '正在读取需求档案')"
      description="从需求背景、所需能力到预期成果，保留一次乡村提问与青年回应的完整上下文。"
      icon="voice"
      :metric="demand?.status || 'SYNC'"
      metric-label="当前响应状态"
      demo
    />
    <div class="museum-content-shell detail-shell">
      <!-- 面包屑 + 返回 -->
      <nav class="crumb">
        <RouterLink to="/voice"><AppIcon name="voice" :size="14" />乡村之声</RouterLink>
        <span class="sep">/</span>
      ...
      <!-- 加载态 -->
      <MuseumState v-if="loading" type="loading" title="正在读取需求详情" description="正在同步需求档案和相关推荐。" />

      <!-- 错误 / 404 -->
      <MuseumState v-else-if="error" type="error" title="需求档案载入失败" :description="error">
      ...
      <template v-else-if="demand">
        <header class="head" :class="statusClass(demand.status)">
          ...
          <h1>{{ demand.title }}</h1>
          <p class="sub">
            <span><AppIcon name="map-pin" :size="14" />{{ demand.town }} · {{ demand.village }}</span>
            <span v-if="demand.publishTime">　·　{{ demand.publishTime }} 发布</span>
            <span class="counts"><AppIcon name="eye" :size="14" />{{ demand.views }}　<AppIcon name="star" :size="14" />{{ demand.favorites }}</span>
          </p>
        </header>
            ...
            <div class="side-card">
              ...
              <div class="mini-actions">
                <button class="count-btn" @click="toggleLike">
                  <AppIcon name="heart" :size="14" />{{ liked ? '已点赞' : '点赞' }}
                </button>
                <button class="count-btn" @click="toggleFav">
                  <AppIcon name="star" :size="14" />{{ faved ? '已收藏' : '收藏' }}
                </button>
              </div>
            </div>
          </aside>
        ...
        <section v-if="related.length" class="related">
          ...
          <div class="related-grid">
            ...
            >
              ...
              <p class="rc-meta"><AppIcon name="map-pin" :size="13" />{{ r.town }} · {{ r.village }}</p>
            </RouterLink>
          </div>
        </section>
      </template>
...
.detail-shell { max-width: 1120px; }

/* 面包屑 */
.crumb { display: flex; align-items: center; gap: .5rem; font-size: .85rem; margin-bottom: 1.4rem; color: var(--color-text-light); }
.crumb a { display: inline-flex; align-items: center; gap: .35rem; color: var(--color-primary); text-decoration: none; }
...
/* 头部 */
.head { position: relative; padding: 1.4rem 1.6rem; background: var(--color-card); border: 1px solid var(--color-border); border-radius: var(--radius-sm); box-shadow: var(--shadow-sm); border-left: 4px solid var(--color-border); }
...
.sub, .sub > span, .counts { display: flex; flex-wrap: wrap; align-items: center; gap: .35rem; }
.sub { margin: .6rem 0 0; gap: .6rem; font-size: .88rem; color: var(--color-text-secondary); }
...
.main { flex: 2; min-width: 300px; background: var(--color-card); border: 1px solid var(--color-border); border-radius: var(--radius-sm); padding: 1.4rem 1.6rem; box-shadow: var(--shadow-sm); }
...
.side-card { position: sticky; top: 1rem; background: var(--color-card); border: 1px solid var(--color-border); border-radius: var(--radius-sm); padding: 1.3rem 1.4rem; box-shadow: var(--shadow-sm); }
...
.count-btn { display: inline-flex; align-items: center; justify-content: center; gap: .35rem; flex: 1; border: 1px solid var(--color-border); background: var(--color-bg); color: var(--color-text-secondary); padding: .45rem; border-radius: var(--radius-sm); font-size: .82rem; cursor: pointer; transition: all var(--transition); }
...
.related-card { position: relative; display: block; padding: 1.1rem 1.2rem; background: var(--color-card); border: 1px solid var(--color-border); border-radius: var(--radius-sm); box-shadow: var(--shadow-sm); text-decoration: none; transition: transform var(--transition), box-shadow var(--transition); }
...
.rc-meta { display: flex; align-items: center; gap: .35rem; margin: 0; font-size: .8rem; color: var(--color-text-secondary); }
</style>
