<template>
  <section class="voice-page museum-public-page">
    <MuseumPageHero
      archive-no="FIELD RESPONSE · VOICE"
      kicker="乡村之声 / 需求协作"
      title="乡村出题，青年回应"
      description="乡镇发布真实需求，高校团队精准响应；实践经验继续沉淀为可检索、可复用的公共知识。"
      icon="voice"
      :metric="totalCount"
      metric-label="条当前演示需求"
      demo
    >
      <template #actions>
        <button class="museum-action-primary" type="button" @click="onPublish">
          <AppIcon name="upload" :size="16" />发布需求
        </button>
      </template>
    </MuseumPageHero>

    <div class="museum-content-shell voice-shell">

      <!-- 窄屏筛选按钮 -->
      <button class="filter-toggle" aria-label="打开筛选" @click="filterOpen = true">
        <AppIcon name="search" :size="16" />筛选 / 搜索<template v-if="hasActiveFilter"> · 已启用</template>
      </button>
      ...
      <div class="voice-body">
        ...
        <aside class="filter-sidebar" :class="{ open: filterOpen }" aria-label="筛选">
          <div class="sidebar-head">
            ...
            <button class="sidebar-close" aria-label="关闭筛选" @click="filterOpen = false"><AppIcon name="close" :size="17" /></button>
          </div>
        ...
        <div class="result-area">
          ...
          <div v-if="activeChips.length" class="crumbs">
            <button v-for="(chip, i) in activeChips" :key="i" class="crumb" @click="removeChip(chip)">
              {{ chip.label }} <AppIcon name="close" :size="12" />
            </button>
          ...
          <MuseumState v-if="loading" type="loading" title="正在读取乡村需求" description="正在连接本地需求档案。" />
          <MuseumState v-else-if="loadError" type="error" title="需求载入失败" :description="`${loadError}，请刷新重试。`" />
            ...
            >
              <div class="demand-main">
                ...
                <div class="demand-meta">
                  <span class="meta"><AppIcon name="map-pin" :size="14" />{{ d.town }} · {{ d.village }}</span>
                  <span class="meta"><AppIcon name="tag" :size="14" />{{ d.majors.join(' / ') }}</span>
                </div>
          ...
          <MuseumState v-else type="empty" title="没有匹配的需求" description="试试调整筛选条件或搜索关键词。" />
        </div>
      </div>

      <!-- 问答互动区 -->
      <section class="qa-section">
        <MuseumSectionHeader index="02" kicker="FIELD KNOWLEDGE BASE" title="问答互动区" icon="message">
            ...
            <div class="qa-answers">
              <div v-for="a in q.answers" :key="a.id" class="qa-answer">
                ...
                <div class="a-body">
                  ...
                  <button class="a-like" @click="likeAnswer(a)"><AppIcon name="heart" :size="13" />{{ a.likes }}</button>
                </div>
              </div>
            </div>
...
.filter-sidebar {
  ...
  border-radius: var(--radius-sm); padding: 1.2rem 1.2rem 1.4rem; box-shadow: var(--shadow-sm);
}
...
/* 搜索 */
.search-bar { display: flex; align-items: center; gap: .5rem; padding: .2rem .9rem; background: var(--color-bg); border: 1px solid var(--color-border); border-radius: var(--radius-sm); }
...
.fchip { padding: .32rem .9rem; border: 1px solid var(--color-border); border-radius: var(--radius-sm); background: var(--color-bg); color: var(--color-text-secondary); font-size: .8rem; cursor: pointer; transition: all var(--transition); }
...
.meta { display: inline-flex; align-items: center; gap: .35rem; font-size: .85rem; color: var(--color-text-secondary); }
...
/* —— 窄屏筛选按钮 + 抽屉 —— */
.filter-toggle { display: none; align-items: center; justify-content: center; gap: .5rem; width: 100%; margin-bottom: 1rem; padding: .7rem 1rem; border: 1px solid var(--color-border); border-radius: var(--radius-sm); background: var(--color-card); color: var(--color-primary-dark); font-size: .9rem; font-weight: 600; cursor: pointer; box-shadow: var(--shadow-sm); }
